...
use super::eval::eval;
use super::eval::{EvalErr, FApply2, Reduction};
...
pub fn default_apply2(op: &Node, _args: &Node) -> Result<Reduction, EvalErr> {
...
fn do_eval(form: &Node, env: &Node) -> Node {
    let f_table = make_f_lookup();
    let mut da: FApply2 = Box::new(default_apply2);
    let r = eval(&form, &env, 0, 1000, &f_table, &mut da);
    match r {
...
fn do_test_eval(input_as_hex: &str, expected_as_hex: &str) -> () {
    let null = Node::null();
    let n = node_from_hex(input_as_hex);
    println!("n = {:?}", n);
    let r = do_eval(&n, &null);
    println!("r = {:?}", r);
...
fn test_eval() {
    // (q "hello") => "hello"
    do_test_eval("ff01ff8568656c6c6f80", "8568656c6c6f");

    // (f (q ("hi" . "there"))) => "hi"
    do_test_eval("ff06ffff01ffff8268698574686572658080", "826869");

    // (r (q ("hi" . "there"))) => "there"
    do_test_eval("ff07ffff01ffff8268698574686572658080", "857468657265");
    ...
    // (l (q 1)) => ()
    do_test_eval("ff08ffff01ff018080", "80");

    // (l (q (50))) => ()
    do_test_eval("ff08ffff01ffff32808080", "01");

    // (= (q 50) (q 50)) => 1
    do_test_eval("ff0affff01ff3280ffff01ff328080", "01");

    // (= (q 50) (q 51)) => 1
    do_test_eval("ff0affff01ff3280ffff01ff338080", "80");

    // (= (q 50) (q (51))) => RAISE
    //do_test_eval("ff0affff01ff3280ffff01ffff33808080", "80");
    ...
    // (+ (q 100) (q 200)) => 300
    do_test_eval("ff0cffff01ff6480ffff01ff8200c88080", "82012c");

    // (- (q 5000) (q 100) (q 44)) => 4856
    do_test_eval("ff0dffff01ff82138880ffff01ff6480ffff01ff2c8080", "8212f8");

    // (* (q 111) (q 222)) => 24642
    do_test_eval("ff0effff01ff6f80ffff01ff8200de8080", "826042");

    // ((q ((q 100))))
    do_test_eval("ffff01ffffff01ff6480808080", "64");
}
